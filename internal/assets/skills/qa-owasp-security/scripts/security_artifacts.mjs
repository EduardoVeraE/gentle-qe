#!/usr/bin/env node
// security_artifacts.mjs — CLI to scaffold OWASP security artifacts from templates.
//
// Usage:
//   node security_artifacts.mjs list
//   node security_artifacts.mjs help [<template>]
//   node security_artifacts.mjs create <template> --out <dir> [--<placeholder> <value> ...] [--strip-hints]
//   node security_artifacts.mjs --self-test
//
// Make executable:
//   chmod +x security_artifacts.mjs
//
...
const DESCRIPTIONS = {
  "security-test-plan": "Security testing plan: scope, threat actors, tools, schedule, exit criteria.",
  "threat-model": "STRIDE + DREAD threat model with DFD and trust boundaries.",
  "vuln-report": "Single vulnerability report with CVSS, CWE, repro steps, remediation.",
  "pentest-report": "Full penetration-test engagement report with findings and recommendations.",
  "security-checklist": "Per-OWASP-category gate (Web Top 10, API Top 10, Mobile Top 10).",
};
...
function generalHelp() {
  console.log(`security_artifacts — scaffold OWASP security artifacts from templates.
...
function cmdHelp(template) {
  ...
  console.log("");
  console.log(`Usage:`);
  console.log(`  node security_artifacts.mjs create ${template} --out ./out \\`);
...
function selfTest() {
  ...
  const templates = listTemplates();
  log(templates.length === 5, `list templates → found ${templates.length} (expected 5)`);

  const tmp = mkdtempSync(join(tmpdir(), "security-artifacts-"));
  try {
    for (const t of templates) {
