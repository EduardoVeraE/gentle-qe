...
func TestPiCodeGraphUnselectedIsNoOp(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(home, branding.StateDir, "pi-codegraph.json")); !os.IsNotExist(err) {
...
func TestPiCodeGraphRootValidationRejectsUnsafeRoots(t *testing.T) {
	...
	// git rev-parse --show-toplevel resolves symlinks in the path it prints.
	// On macOS, t.TempDir() returns a path under /var/folders, itself a
	// symlink to /private/var/folders, so compare against the canonicalized
	// form of valid rather than the raw string — unrelated to the QE
	// overlay, this is a portability gap in the plain string comparison.
	wantTopLevel, evalErr := filepath.EvalSymlinks(valid)
	if evalErr != nil {
		t.Fatalf("EvalSymlinks(%q) error = %v", valid, evalErr)
	}
	if output, err := exec.Command("git", "-C", valid, "rev-parse", "--show-toplevel").CombinedOutput(); err != nil || strings.TrimSpace(string(output)) != wantTopLevel {
		t.Fatalf("git -C root resolution = %q, %v; want %q", output, err, wantTopLevel)
	}
	t.Chdir(home)
...
func TestPiCodeGraphFailureRestoresNewMCPAndChild(t *testing.T) {
	...
	piCodeGraphAtomicWrite = func(path string, data []byte, mode os.FileMode) (filemerge.WriteResult, error) {
		if path == filepath.Join(home, branding.StateDir, "pi-codegraph.json") {
...
func TestPiCodeGraphPendingProbePreservesConfiguredFiles(t *testing.T) {
	...
	mcpPath := filepath.Join(home, ".pi", "agent", "mcp.json")
	childPath := filepath.Join(home, ".pi", "agent", "subagents", "worker.md")
	manifestPath := filepath.Join(home, branding.StateDir, "pi-codegraph.json")
	writePiFile(t, childPath, "---\ntools: bash\n---\nwork\n")
...
func TestPiCodeGraphPendingProbeRejectsConflictingChildAndRollsBack(t *testing.T) {
	...
	if _, statErr := os.Stat(filepath.Join(home, branding.StateDir, "pi-codegraph.json")); !os.IsNotExist(statErr) {
...
func TestPiCodeGraphUninstallFailsClosedOnChildReadFailure(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(home, branding.StateDir, "pi-codegraph.json")); err != nil {
  ...
  case "$request" in
    ...
    *'"id":2'*) while :; do :; done ;;
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 100ms was too tight on a loaded machine: the "tools list" case
			// needs the initialize round-trip (spawn the fake CodeGraph
			// script + write + read) to finish, THEN start spinning on the
			// tools/list phase, before the deadline fires — otherwise the
			// error is misclassified as "MCP initialize" even though
			// initialize itself succeeded. The busy-loop fake script
			// (`while :; do :; done`) also competes for CPU with the test
			// process, widening spawn/read latency further. 3s keeps this
			// well clear of that budget without meaningfully slowing the
			// suite (both cases still resolve almost immediately once the
			// classified phase starts spinning).
...
func TestPiCodeGraphPreservesSensitiveFileModes(t *testing.T) {
	...
	for _, tt := range []struct {
		path string
		want os.FileMode
	}{{mcpPath, 0o600}, {childPath, 0o640}, {filepath.Join(home, branding.StateDir, "pi-codegraph.json"), 0o600}} {
